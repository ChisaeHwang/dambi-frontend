...
const AppTitleBar: React.FC = () => {
  ...
  // 일렉트론 환경인지 확인
  const electronAvailable = isElectronEnv();

  useEffect(() => {
    const checkMaximized = async () => {
      if (electronAvailable && window.electron?.isMaximized) {
        try {
  ...
  const handleMinimize = () => {
    if (electronAvailable && window.electron?.minimize) {
      window.electron.minimize();
    }
  };

  const handleMaximize = () => {
    if (electronAvailable && window.electron?.maximize) {
      window.electron.maximize();
    }
  };

  const handleClose = () => {
    if (electronAvailable && window.electron?.close) {
